...
var fs = require('fs');
var database = require("./data router/database");       //handles communication with ClassRep database
...
var dbConn = mysql.createConnection({
  host: "mannschaft-knust.mysql.database.azure.com",
  user: "mannschaft-db@mannschaft-knust",
  password: "M@nnschaft",
  database: "mannschaft_classrep"
});

dbConn.connect(function(err) {
  if (err){
    fs.appendFileSync('serverlog', "Database Inaccessible! Is it up?\n");
    fs.appendFileSync('serverlog',err);
  }
  else {
    fs.appendFileSync('serverlog', "Connected to ClassRep Database!\nListening\n");
  }
});
